from data import SIAT_PCQDModule
...
def train(configs, k):
    ...
    datamodule = SIAT_PCQDModule(pd_db = info_file, path_ply = path_ply, number_patches = configs["number_patches"], points_structure=configs['points_structure'], batch_size = configs["batch_size"], fold = k)
    wandb_logger = WandbLogger(project="NRPCQA", config = configs,  name=configs["name"] + "Fold {}".format(k), tags = ["SIAT"])
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath = "best_models_SIAT/Fold{}".format(k), 
        filename = "PST_PCQAModule-SIAT-GVP-"+str(k)+"Fold-{epoch:02d}-{val/loss_mse:.4f}",  # <--- note epoch suffix here
        save_last = True, 
        every_n_epochs = 1,
        save_top_k = 10,
    ...
    shutil.rmtree("best_models_SIAT/Fold{}".format(k))
    os.mkdir("best_models_SIAT/Fold{}".format(k))
...
def test(trained_model:PST_PCQAModule, datamodule, trainer:Trainer, configs:dict, k:int):
    # for metrics 
    list_checkpoints = scan_all_dir("best_models_SIAT/Fold{}".format(k))
    best_PLCC = -1000
    best_checkpoint = ""
...
if __name__ == "__main__":
    ...
    np.savez("data/SIAT-PCQD/all_PLCC", all_PLCC)
    np.savez("data/SIAT-PCQD/all_SRCC", all_SRCC)
    np.savez("data/SIAT-PCQD/all_KRCC", all_KRCC)
    np.savez("data/SIAT-PCQD/all_RMSE", all_RMSE)

    wandb_logger = WandbLogger(project="NRPCQA", config = configs,  name=configs["name"] + "All results", tags = ["SIAT", "GVP"])
